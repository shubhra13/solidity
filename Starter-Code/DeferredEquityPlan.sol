...
contract DeferredEquityPlan {
    ...
    // testing: uint start_time = fakenow; // permanently store the time this contract was initialized

    // Set the `unlock_time` to be 365 days from now
    uint unlock_time =  now + 365;
    // testing: uint unlock_time =  fakenow + 365;
    ...
    function distribute() public {
        ...
        require(unlock_time <= now, "Equity is not yet vested!");
        //testing: require(unlock_time <= fakenow, "Equity is not yet vested!");
        
        // `distributed_shares` is less than the `total_shares`
        require(distributed_shares < total_shares, "Share distributed cannot exceed total shares");
        ...
        //testing: distributed_shares = (fakenow - start_time) / 365 * annual_distribution;
